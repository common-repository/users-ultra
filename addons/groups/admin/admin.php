...
$sql = ' SELECT count(*) as total FROM ' . $wpdb->prefix . 'usersultra_groups_users_groups_rel WHERE group_user_group_rel_group_id= "'.$group_id.'"  ' ;
	...
	function admin_init() 
	{
		...
		$this->tabs = array(
			'manage' => __('Manage Groups','users-ultra')
	...
	public function editPostContent($oPost)
    {
       ...
       $html .= '  <label for="'.$oUUltraUserGroup->group_name.'-'.$oUUltraUserGroup->group_id.'" class="selectit" style="display:inline;" >
            '.$oUUltraUserGroup->group_name.$sAddition.'
				...
				$html .= "<a href='admin.php?page=uultra-groups'>".__('Please create a user group first.','users-ultra')."</a>";
    ...
    {
        $aDefaults['uultra_access'] = __('Users Ultra Access', 'users-ultra');
        return $aDefaults;
    }
	...
	public function edit_group_conf ()
	{
		...
		if(isset($_POST['group_name']) && $_POST['group_name']!="")
		{
			...
			echo '<div class="updated"><p><strong>'.__('The group has been updated.','users-ultra').'</strong></p></div>';
		}else{
			
			echo '<div class="error"><p><strong>'.__('Please input a name.','users-ultra').'</strong></p></div>';
	...
	function add_menu()
	{
		add_submenu_page( 'userultra', __('Groups','users-ultra'), __('Groups','users-ultra'), 'manage_options', 'uultra-groups', array(&$this, 'admin_page') );
	...
	public function save()
	{
		...
		if(isset($_POST['group_name']) && $_POST['group_name']!="")
		{
			...
			echo '<div class="updated"><p><strong>'.__('New group has been created.','users-ultra').'</strong></p></div>';
		}else{
