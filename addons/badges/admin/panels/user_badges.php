...
<h3><?php _e('View/Delete Member Medallions','users-ultra'); ?></h3>
<table class="form-table">
	<tr valign="top">
		<th scope="row"><label for="badge_user"><?php _e('Find a Member','users-ultra'); ?></label></th>
		<td>
			<select name="badge_user" id="badge_user" class="chosen-select" style="width:300px" data-placeholder="">
				<option value=""><?php _e('Select Member...','users-ultra'); ?></option>
				<?php
...
<p class="submit">
	<input type="submit" name="find-user-badges" id="find-user-badges" class="button button-primary" value="<?php _e('Find Member Medallions','users-ultra'); ?>"  />
</p>
...
if (isset($_POST['badge_user']) && $xoouserultra->user_exists($_POST['badge_user']) ) {
	...
	{
		echo '<h3>'.sprintf(__('%s\'s Medallions','users-ultra'),  $xoouserultra->userpanel->get_user_meta_custom($user_id, 'display_name') ).'</h3>';
		foreach($badges as $k => $arr) {
			...
			<div class="uultra-user-badge">
				...
				<a href="#" class="button uultra-delete-badge" data-user="<?php echo $user_id; ?>" data-url="<?php echo $arr['badge_url']; ?>"><?php _e('Delete Medallion','users-ultra'); ?></a>
			</div>
		...
		echo '<p>'.__('This Member does not have any manually assigned medallions.','users-ultra').'</p>';
	}
