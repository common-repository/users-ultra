...
class uultra_dg_api {
	...
	function find_badges( $form ) {
		...
		extract($form);
		if ($badge_user == '') {
			$result['error'] = __('You did not choose a member.','users-ultra');
		}
		return $result;
	}
	...
	function new_badge( $form ) {
		...
		if ($badge_method == 'manual') {
			if (!$badge_url) {
				$result['error'] = __('You must choose a medallion first.','users-ultra');
			} else if (!$badge_title) {
				$result['error'] = __('You must enter a title for the medallion.','users-ultra');
			} else {
			
				if (isset($badge_to_users) && is_array($badge_to_users)){
					$this->give_badge_to_users( $form );
					echo '<div class="updated"><p><strong>'.__('Medallions have been assigned.','users-ultra').'</strong></p></div>';
				}
		...
		if ($badge_method == 'achievement') {
			if (!$badge_url) {
				...
				$result['error'] = __('You did not select any number for this fulfillment.','users-ultra');
			} else {
		...
		if ($badge_method == 'points') {
			if (!$badge_url) {
				...
				$result['error'] = __('You must enter a required number of points for this medallion.','users-ultra');
			} else {
