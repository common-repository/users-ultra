...
class XooSocial
{
	...
	public function send_unfollow_request()
	{
		...
		if(isset($logged_user_id) && $receiver_id >0 && $logged_user_id >0 )
		{
			...
			echo __("You're not following this user anymore ", 'users-ultra');
					
		}else{
					
			echo __("You have to be logged in to unfollow this user ", 'users-ultra');
		...
		if($logged_user_id!=$receiver_id)
		{
			...
			if($this->check_if_following($receiver_id)) {
				...
				if(isset($logged_user_id) && $receiver_id >0 && $logged_user_id >0 )
				{
					...
					echo __(" Following ", 'users-ultra');
					
				}else{
					
					echo __("You have to be logged in to follow this user ", 'users-ultra');
				...
				echo __("Already Following", 'users-ultra');
			...
			echo __("You cannot follow yourself!", 'users-ultra');
	...
	public function send_friend_request()
	{
		...
		if($this->check_if_sent($receiver_id )) {
			...
			if(isset($logged_user_id) && $logged_user_id >0 && $logged_user_id >0 )
			{
				...
				if($logged_user_id==$receiver_id)
				{
					echo __(" You can't be your own friend. ", 'users-ultra');
					...
					echo __(" Friend Request Sent ", 'users-ultra');
				...
				echo __("You have to be logged in to send a friend request ", 'users-ultra');
			...
			echo __("Request Already Sent", 'users-ultra');
	...
	public function like_item()
	{
		...
		{		
			...
			if( (isset($logged_user_id) && $logged_user_id >0) || ($guest_allowed ==1 && $item_id >0))
			{
				...
				echo __("Thanks ", 'users-ultra');
				
			}else{
				
				echo __("Please login to rate ", 'users-ultra');
			...
			echo __("You've already liked it ", 'users-ultra');
	...
	public function get_friends($user_id)
	{
		...
		$html .= '<a class="uultra-btn-friend-directory" id="uu-send-friend-request" href="#" user-id="'.$user_id.'" title="'.__('Send Friend Request','users-ultra').'"><span><i class="fa fa fa-users fa-lg"></i> '.__('Be a Friend', 'users-ultra').'</span> </a>';
	...
	public function get_follow_button($user_id)
	{
		...
		{
			$html .= '<a class="uultra-btn-follow-request"  href="#" user-id="'.$user_id.'" title="'.__('Follow User','users-ultra').'"><span><i class="fa fa fa-plus fa-lg"></i> <i class="fa fa fa-user fa-lg"></i>  '.__('Follow', 'users-ultra').'</span> </a>';
		
		}else{
			
			$html .= '<a class="uultra-btn-unfollow-request"  href="#" user-id="'.$user_id.'" title="'.__('Unfollow User','users-ultra').'"><span><i class="fa fa fa-plus fa-lg"></i> <i class="fa fa fa-user fa-lg"></i>  '.__('Unfollow', 'users-ultra').'</span> </a>';
	...
	public function get_follow_widget($user_id)
	{
		...
		$html .= '<a class="uultra-btn-follow" id="uu-follow-request" href="#" user-id="'.$user_id.'" title="'.__('Send Friend Request','users-ultra').'"><span><i class="fa fa fa-eye fa-lg"></i>'.__('Follow', 'users-ultra').'</span> </a>';
	...
	public function friend_request_action()
	{
		...
		if($item_action=='approve')
		{
			...
			$message = __('Request approved','users-ultra'); 	
		}
		...
		if($item_action=='deny')
		{
			...
			$res = $wpdb->query( $sql );			
			$message = __('Request rejected','users-ultra'); 	
		}	
	...
	public function get_item_likes($item_id, $module)
	{
		...
		$total = 0;
		 ...
		 if($total==1)
		 {
			 $like_text = __('Like','users-ultra');
		 
		 }else{
			 
			  $like_text = __('Likes','users-ultra');
		...
		$html .= '<a class="uultra-btn-like" id="uu-like-item" href="#" item-id="'.$item_id.'" data-module="'.$module.'" data-vote="1" title="'.__('Like','users-ultra').'"><span><i class="fa fa fa-thumbs-o-up fa-lg"></i></span> </a> <a class="uultra-btn-like" id="uu-like-item" href="" title="'.__('Dislike','users-ultra').'" item-id="'.$item_id.'" data-module="'.$module.'" data-vote="-1"><span><i class="fa fa fa-thumbs-o-down fa-lg"></i></span> </a>';
	...
	public function get_item_likes_amount_only()
	{
		...
		if(isset($_POST["module"]))
		{
		 ...
		 if($total==1)
		 {
			 echo $total." ".__('Like','users-ultra');
		 
		 }else{
			 
			 echo $total." ".__('Likes','users-ultra');
	...
	function show_friend_request()		
	{
		...
		$html .= "<h3>".__('Latest Friend Requests','users-ultra')."</h3> ";
		...
		$html.='		</div>
				...
				<table class="widefat fixed" id="table-3" cellspacing="0">
					...
					$html .= '<tr >
							...
							<td><a class="uultra-btn-denyapprove" id="uu-approvedeny-friend" href="#" item-id="'.$request_id.'" action-id="approve" title="'.__('Approve','users-ultra').'"><span><i class="fa fa fa-thumbs-o-up fa-lg"></i></span> '.__('Accept', 'users-ultra').' </a> <a class="uultra-btn-denyred" id="uu-approvedeny-friend" href="" title="'.__('Deny','users-ultra').'" item-id="'.$request_id.'" action-id="deny"><span><i class="fa fa fa-thumbs-o-down fa-lg"></i></span> '.__( 'Deny', 'xoousers' ).'</a></td>
						</tr>';
	...
	function show_all_my_friends()		
	{
		...
		$html .= "<h3>".__('All My Friends','users-ultra')."</h3> ";
		...
		$html.='		</div>
				...
				<table class="widefat fixed" id="table-3" cellspacing="0">
					...
					$html .= '<tr>
							...
							<td><a class="uultra-btn-denyred" id="uu-approvedeny-friend" href="" title="'.__('Block','users-ultra').'" item-id="'.$request_id.'" action-id="block"><span><i class="fa fa fa-thumbs-o-down fa-lg"></i></span> '.__('Block', 'users-ultra').' </a></td>
						</tr>';
