...
class XooCustomizer
{
	...
	function get_custom_user_avatar_admin() 
	{
		...
		{			
			...
			$html .= '<input type="button" name="submit"  class="button button-secondary " id="uultradmin-remove-custom-user-avatar-image"   value="'.__('Remove','users-ultra').'"  />';		
	...
	function get_custom_bg_for_user_profile_admin() 
	{
		...
		{			
			...
			$html .= '<input type="button" name="submit"  class="button button-secondary " id="uultradmin-remove-custom-user-bg-image"   value="'.__('Remove','users-ultra').'"  />';		
			...
			foreach ($_FILES as $key => $array) {				
				...
				if ($name) {					
					...
					if ( !in_array($ext, $this->allowed_extensions) )
					{
						...
						$this->messages_process .= __('--- Finished ---  ', 'users-ultra');
	...
	public function set_template_list()
	{
		...
		$tabs[1] =array(
			  ...
			  'title' => __('Fancy Template 3 Columns', 'users-ultra'),
			  'description' => __('This is the default template of Users Ultra PRO. This is a widgetized template that can be easily customized by the users.', 'users-ultra'),												
			  'visible' => 1
		);
		...
		$tabs[3] =array(
			  ...
			  'title' => __('Fancy Template 2 Columns', 'users-ultra'),
			  'description' => __('This template will display all the widgets in two columns.', 'users-ultra'),												
			  'visible' => 1
		...
		$tabs[4] =array(
			  ...
			  'title' => __('Fancy Template 1 Column', 'users-ultra'),
			  'description' => __('This template will display all the widgets in one column', 'users-ultra'),												
			  'visible' => 1
		...
		$tabs[2] =array(
			  ...
			  'title' => __('Default Template', 'users-ultra'),
			  'description' => __('This template is the same than Lite version. Widgets are not available on this template. Some features may not be available.', 'users-ultra'),												
			  'visible' => 1
	...
	public function customizer_set_default_template()
	{
        ...
        if($xoouserultra->check_is_pro_version()){
	...
	function uuultra_default_users_layout()
	{
		...
		$tabs[1] =array(
			  'position' => 1,
			  'icon' => 'fa-credit-card',			
			  'title' => __('Basic Information', 'users-ultra'),
		...
		$tabs[2] =array(
			  'position' => 2,
			  'icon' => 'fa-users',			
			  'title' => __('My Friends', 'users-ultra'),
			  'type' =>1, // 1-text, 2-shortcode
			  'editable' =>0,					
			  'native' =>1,
			  'content' =>NULL,					
		...
		$tabs[3] =array(
			  'position' => 3,
			  'icon' => 'fa-camera',			
			  'title' => __('My Photos', 'users-ultra'),
			  'type' =>1, // 1-text, 2-shortcode
			  'editable' =>0,	
			  'native' =>1,
			  'content' =>NULL,									
		...
		$tabs[4] =array(
			  'position' => 4,
			  'icon' => 'fa-picture-o',			
			  'title' => __('My Galleries', 'users-ultra'),
			  'type' =>1, // 1-text, 2-shortcode
			  'editable' =>0,	
			  'native' =>1,
			  'content' =>NULL,									
		...
		$tabs[5] =array(
			  ...
			  'title' => __('My Latest Posts', 'users-ultra'),	
			  'type' =>1, // 1-text, 2-shortcode
			  'editable' =>0,	
			  'native' =>1,
			  'content' =>NULL,									
		...
		$tabs[6] =array(
			  'position' =>6,
			  'icon' => 'fa-eye',			
			  'title' => __('Followers', 'users-ultra'),	
			  'type' =>1, // 1-text, 2-shortcode
			  'editable' =>0,
			  'native' =>1,
			  'content' =>NULL,										
		...
		$tabs[8] =array(
			  'position' =>8,
			  'icon' => 'fa-film',			
			  'title' => __('My Latest Videos', 'users-ultra'),
			  'type' =>1, // 1-text, 2-shortcode
			  'editable' =>0,
			  'native' =>1,
			  'content' =>NULL,											
		...
		$tabs[10] =array(
			  'position' =>10,
			  'icon' => 'fa-file-text',			
			  'title' => __('About / Bio', 'users-ultra'),
			  'type' =>1, // 1-text, 2-shortcode	
			  'editable' =>0,
			  'native' =>1,
			  'content' =>NULL,										
	...
	public function uultra_update_unused_wigets()
	{
        ...
        if($xoouserultra->check_is_pro_version()){
            ...
            $modules = sanitize_text_field($_POST["order"]);  //unused

            $modules_active = sanitize_text_field($_POST["order_active_widgets"]); 	//col 1	
            $modules_active_2 = sanitize_text_field($_POST["order_active_widgets2"]); 	//col 2	
            $modules_active_3 = sanitize_text_field($_POST["order_active_widgets3"]); 	//col 3	
	...
	public function uultra_update_unused_wigets_packages()
	{
        ...
        if($xoouserultra->check_is_pro_version()){
            $module_list = array();
            $modules = sanitize_text_field($_POST["order"]); 		
	...
	public function widget_addition_form_confirm()
	{
		...
		$uu_title = sanitize_text_field($_POST["uu_title"]);
		$uu_type = sanitize_text_field($_POST["uu_type"]);
		$uu_editable = sanitize_text_field($_POST["uu_editable"]);
		$uu_content = sanitize_text_field($_POST["uu_content"]);
	...
	public function uultra_load_links_addition_form()
	{
		...
		$html .= '<table width="100%" border="0" cellspacing="2" cellpadding="3">
		  ...
		  <tr>
			...
			$html .= ' <p class="submit">
					<input type="button" name="submit"  class="button  uultra-links-add-new-close"   value="'.__('Close','users-ultra').'"  />  <input type="button" name="submit"  class="button button-primary uultra-links-add-new-confirm"   value="'.__('Submit','users-ultra').'"  /> <span id="uultra-add-new-links-m-w" ></span>
				</p> ';
	...
	public function uultra_load_wdiget_addition_form()
	{
		...
		<tr>
				<td width="50%"> '.__('Editable by user:','users-ultra').'</td>
				<td width="50%">
				<select name="uultra_add_mod_widget_editable"  id="uultra_add_mod_widget_editable" size="1">
				 
				  <option value="0" selected="selected">'.__("NO ",'users-ultra').'</option>
				  <option value="1">'.__("YES",'users-ultra').'</option>
				</select>
			...
			$html .= ' <p class="submit">
					<input type="button" name="submit"  class="button  uultra-widgets-add-new-close"   value="'.__('Close','users-ultra').'"  />  <input type="button" name="submit"  class="button button-primary uultra-widgets-add-new-confirm"   value="'.__('Submit','users-ultra').'"  /> <span id="uultra-add-new-widget-m-w" ></span>
				</p> ';
	...
	function get_admin_widget_template_membership($widget, $key, $package_id, $unused=false)
	{
		...
		$widget_title = $widget["title"];
			...
			if(!$this->uultra_check_if_unused_widget_settings($key, $package_id, $unused) )
			{
				...
				$html .= '<table width="100%" border="0" cellspacing="2" cellpadding="3">
				<tr>
					<td width="50%"> '.__("Widget's Title ",'users-ultra').'</td>
			  ...
			  <tr>
					<td width="50%"> '.__('Header Background Color','users-ultra').'</td>
				  ...
				  <tr>
					<td>'.__('Background Color','users-ultra').'</td>
				   ...
				   <tr>
					<td>'.__('Header Text Color','users-ultra').'</td>				
				  ...
				  <tr>
					<td>'.__('Widget Text Color','users-ultra').'</td>
			...
			//	print_r($widget);
				...
				$html .= ' <p class="submit">
						<input type="button" name="submit"  class="button button-primary uultra-widget-customizer-save-membership"  data-widget="'.$key.'" value="'.__('Save Changes','users-ultra').'"  /> <span id="uultra-widget-update-message-'.$key.'"></span>
					</p> ';
				...
				//check if this is a custom widget				
					if( isset($widget["native"]) && $widget["native"] ==0)
					{
						 $html .= '<p style="text-align:right;"><a href="#" class="uultra-delete-custom-widget" data-widget="'.$key.'" title='.__('delete','users-ultra').'>'.__('delete','users-ultra').'</a></p>';
		...
		$widget_title = $widget["title"];
			...
			if(!$this->uultra_check_if_unused_widget($key, $unused) )
			{
			  ...
			  <tr>
				...
				$html .= $this->get_widget_extra_settings($key);            
				$html .= ' <p class="submit">
						<input type="button" name="submit"  class="button button-primary uultra-widget-customizer-save"  data-widget="'.$key.'" value="'.__('Save Changes','users-ultra').'"  /> <span id="uultra-widget-update-message-'.$key.'"></span>
					</p> ';
		...
		$html .= '<table width="100%" border="0" cellspacing="2" cellpadding="3">
		  ...
		  <tr>
				...
				<select name="uultra_add_mod_widget_type_'.$key.'"  id="uultra_add_mod_widget_type_'.$key.'" size="1">
				  ...
				  <option value="1" '.$type_1.'>'.__("Text: ",'users-ultra').'</option>
	...
	public function get_widget_extra_settings($widget)
	{
		...
		if($widget==5)
		{
			...
			$html .= ' <h4>'.__('Post Types','users-ultra').'</h4>            
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
                <td width="50%">'.__('Display Post Types','users-ultra').'</td>
	...
	public function customizer_widget_appearance()
	{
		...
		$array_customizer = array();		
		$widget_id = sanitize_text_field($_POST['p_id']);		
				
		$widget_post_types = "";
		if($widget_id==5) //my posts widgets
		{
			$str_types = "";
			$widget_post_types = sanitize_text_field($_POST['widget_post_type_list']);
			$ii = 0;
		...
		if($this->uultra_check_if_custom_widget($widget_id) && isset($_POST['uultra_add_mod_widget_title'])  && $_POST['uultra_add_mod_widget_title'] !="" )	{
			...
			$custom_widgets[$widget_id]['title'] = sanitize_text_field($_POST['uultra_add_mod_widget_title']);
			$custom_widgets[$widget_id]['type'] = sanitize_text_field($_POST['uultra_add_mod_widget_type']);
			$custom_widgets[$widget_id]['editable'] = sanitize_text_field($_POST['uultra_add_mod_widget_editable']);
			$custom_widgets[$widget_id]['content'] = sanitize_text_field($_POST['uultra_add_mod_widget_content']);				
			...
			$widgets_col[$widget_id]['title'] = sanitize_text_field($_POST['uultra_add_mod_widget_title']);
			$widgets_col[$widget_id]['type'] = sanitize_text_field($_POST['uultra_add_mod_widget_type']);
			$widgets_col[$widget_id]['editable'] = sanitize_text_field($_POST['uultra_add_mod_widget_editable']);
			$widgets_col[$widget_id]['content'] = sanitize_text_field($_POST['uultra_add_mod_widget_content']);	
	...
	public function uultra_internal_user_menu_add_new_link_confirm()
	{
		...
		{
			$error_message = true;
			$html .= __('ERROR: Slug already exists!', 'users-ultra');
		...
		{
			$error_message = true;
			$html .= __('ERROR: Please input the slug!', 'users-ultra');		
		}
		...
		{
			$error_message = true;
			$html .= __('Please input only letters, numbers and - _ !', 'users-ultra');		
		}
	...
	function uultra_get_custom_link_content()
	{
		...
		if(isset($modules[$id]) )
		{
			...
			$html .= __('Content not found', 'users-ultra');
	...
	function uultra_update_user_custom_link_admin()
	{
		...
		if(isset($modules[$id]) && $uu_title!="" && $uu_icon!="")
		{
			...
			$html = __('All fields mandatory!', 'users-ultra');
		}
	...
	{					
		...
		$html .= '<p><strong>'.__('Title', 'users-ultra').'</strong></p>';
		$html .= '<p><input type="text" name="uultra-link-name-'.$key.'" id="uultra-link-name-'.$key.'" value="'.$module['title'].'"></p>';
		
		$html .= '<p><strong>'.__('Slug', 'users-ultra').'</strong></p>';
		$html .= '<p><input type="text" name="uultra-link-slug-'.$key.'" id="uultra-slug-name-'.$key.'" value="'.$module['slug'].'" '.$disable.'></p>';	
		
		$html .= '<p><strong>'.__('Font Awesome Icon', 'users-ultra').'</strong></p>';
		$html .= '<p><input type="text" name="uultra-link-icon-'.$key.'" id="uultra-icon-name-'.$key.'" value="'.$module['icon'].'" ><span><a href="http://fortawesome.github.io/Font-Awesome/icons/" target="_blank">'.__('find more','users-ultra').'</a></span></p>';					
			
			$html .= ' <p class="submit">
					<input type="button" name="submit"  class="button button-primary '.$btn_submit_class.'"   value="'.__('Submit','users-ultra'). '" uultra-linkid = '.$key.'  /> <span id="uultra-add-new-links-m-w-d-'.$key.'" ></span>
				</p> ';	
		...
		//if($module["link_type"] =='custom')
		//{		 
				 $html .= '<p><strong>'.__('Edit Content:', 'users-ultra').'</strong></p>';
				 $html .= '<p>'.__('You can edit what the user will see by clicking on the following button:', 'users-ultra').'</p>';		
				 
				 $html .= ' <p class="submit">
					<input type="button" name="submit"  class="button button-primary uultra-links-edit-text-btn"   value="'.__('Click to edit content','users-ultra'). '" uultra-linkid = '.$key.'  /> <span id="uultra-add-new-links-m-w-text-'.$key.'" ></span>
				</p> ';	
	...
	public function uultra_get_default_modules_array()
	{
		...
		$tabs[0] =array(
			  ...
			  'content_type' => '',
			  'icon' => 'fa-tachometer',			 			
			  'title' => __('Dashboard', 'users-ultra'),											
			  'visible' => 1
		);
		...
		$tabs[2] =array(
			  ...
			  'content' => '',
			  'icon' => 'fa-users',			
			  'title' => __('Following', 'users-ultra'),											
			  'visible' => 0 //not visible in user's dashboard
		);
		...
		$tabs[4] =array(
			  ...
			  'title' => __('My Videos', 'users-ultra'),											
			  'visible' => 1
		);
		...
		$tabs[7] =array(
			  ...
			  'content_type' => '',
			  'icon' => 'fa-user',			
			  'title' => __('My Topics', 'users-ultra'),											
			  'visible' => 0 //not visible in user's dashboard
		);
		...
		$tabs[8] =array(
			  ...
			  'content_type' => '',
			  'icon' => 'fa-envelope-o',			
			  'title' => __('My Messages', 'users-ultra'),											
			  'visible' => 1
		);
		...
		$tabs[9] =array(
			  ...
			  'title' => __('Profile Customizer', 'users-ultra'),											
			  'visible' => 1
		);
		...
		$tabs[10] =array(
			  ...
			  'content_type' => '',
			  'icon' => 'fa-truck',			
			  'title' => __('My Purchases', 'users-ultra'),											
			  'visible' => 1
		);
		...
		$tabs[11] =array(
			  ...
			  'content_type' => '',
			  'icon' => 'fa-list',			
			  'title' => __('My Orders', 'users-ultra'),											
			  'visible' => 1
		);
		...
		$tabs[12] =array(
			  ...
			  'content_type' => '',
			  'icon' => 'fa-user',			
			  'title' => __('My Profile', 'users-ultra'),											
			  'visible' => 1
		);
	...
	function uultra_new_links_add_form()
	{
		...
		$html .= '<table width="100%" border="0" cellspacing="2" cellpadding="3">
		 ...
		 <tr>
				<td width="50%"> '.__("Slug: ",'users-ultra').'</td>
	...
	function  get_customizer_columns_user ()	
	{
		...
		if($cols==1 || $cols==2 || $cols==3)
		{
			...
			$html .= ' <div class="col1" '. $dimension_style.'>    
						<h3 class="colname_widget">'.__('Column 1','users-ultra').'</h3>                           
		...
		if($cols==2 || $cols==3)
		{
			...
			$html .=' <div class="col2" '. $dimension_style.'>  
						<h3 class="colname_widget">'. __('Column 2','users-ultra').'</h3>                             
		...
		if($cols==3)
		{
			...
			$html .='<div class="col3" '. $dimension_style.'> 
					   ...
					   <h3 class="colname_widget">'.__('Column 3','users-ultra').'</h3>                        
	...
	public function uultra_custom_widgets_actions ($user_id, $widget_id)	
	{
		...
		if(!empty($widget))
		{
			...
			$html .=' <li><!--- custom widget--->                 
					...
					$html .='	 <section class="default-bg uultra-profile-widget-arrow-'.$widget_id.' " ></section> ';
						
                    $html .='    <div class="uultra-table-widget-custom" '.$widget_bg_css.'>';
				...
				{								
					...
					if (isset($array[$key]['name']) && $name)
					{
						...
						if($_fsocial==1)
						{
							...
							if($custom_link_text!='')							
							{
								$html_social_link ="<a href='".$this->txt2link($social_meta)."' target='_blank'>".$custom_link_text."</a>";								
							}else{
							
								$html_social_link ="<a href='".$this->txt2link($social_meta)."' target='_blank'>".$social_meta."</a>";		
	...
	public function get_profile_bg($user_id)
	{
		...
		if($user_pic!="")
		{
			...
			$html .= '<div><a href="#" id="uultra-remove-profile-bg" ><img src="'.xoousers_url.'templates/'.xoousers_template.'/img/remove_p.png" class="uultra-remove-bg-ico" title="'.__('Remove image','users-ultra').'" alt="'.__('Remove image','users-ultra').'" /></a></div>';		
				...
				<div id="plupload-upload-ui-profilebg" class="hide-if-no-js">
					...
					<div id="drag-drop-area-profilebg">
                        ...
                        <div  class="uultra-bg-image-box" id="uultra-bg-img-bg-id"><?php echo $this->get_profile_bg($user_id )?></div>
							<p class="drag-drop-info"><?php	_e('Drop Profile Background Image Here', 'users-ultra') ; ?></p>
                          
                            <div style="display:">
							<p><?php _ex('or', 'Uploader: Drop files here - or - Select Files'); ?></p>
                            
                            <div class="uultra-uploader-buttons" id="plupload-browse-button-profilebg">
                            <?php	_e('Select File', 'users-ultra') ; ?>
                            </div>
...
<div class="uultra-uploader-buttons" id="uultra-confirm-image-cropping">
                            <?php	_e('Crop & Save Cover Image', 'users-ultra') ; ?>
                            </div>
...
<div class="uultra-uploader-buttons" id="uultra-confirm-avatar-cropping">
                            <?php	_e('Crop & Save '.$avatar_is_called.' ', 'users-ultra') ; ?> <span class="uultra-please-wait-croppingmessage" id="uultra-cropping-avatar-wait-message"><?php	_e('Please wait ... ', 'users-ultra') ; ?></span>
                            </div>
                            
                            <div class="uultra-uploader-buttons-delete-cancel" id="btn-cancel-avatar-cropping" >
                            <a href="<?php echo $my_account_url."?module=uultra-user-avatar"?>" class="uultra-remove-cancel-avatar-btn"><?php	_e('Cancel', 'users-ultra') ; ?></a>
                            </div>
	...
	{
		$html = '<div id="dialog-form" class="uultra-send-pm-box" title=" '.__("Send Private Message ",'users-ultra').'">
			<p class="validateTips"> '.__("All form fields are required. ",'users-ultra').'</p>
			<form>
			<fieldset>
			<label for="name">'.__("Subject: ",'users-ultra').'</label>
			<input type="text" name="uu_subject" id="uu_subject" class="text ">
			<label for="email">'.__("Message ",'users-ultra').':</label>
