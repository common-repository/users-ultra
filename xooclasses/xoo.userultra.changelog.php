...
class XooChangelog {
	...
	public function show_change_log($atts)
	{
		...
		if ( empty( $rows ) )
		{
			...
			{
				$html.='<ul>';
				$html.='<span class="versionchangelog"> '.__('Version: ','users-ultra').''.$changelog->changelog_version.'</span> - <span class="datechangelog">'.date("m/d/Y", strtotime($changelog->changelog_date)).'</span>';
