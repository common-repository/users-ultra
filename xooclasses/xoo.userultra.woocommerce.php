		...
		if('admin.php' == $pagenow && isset($_GET['page']) && ('import' ==$tab) 
			...
			if ( isset($_REQUEST['sync']) && !isset($_POST['submit']) && !isset($_POST['uultra-add']) && !isset($_POST['reset-options']) && !isset($_POST['reset-options-fields']) ) {
				...
				if(file_exists($this->woocommerce_file_path)){
					...
					//echo __('Woocommerce File Doesn\'t Exist','users-ultra');exit;
				}	
				...
				//echo '<div class="updated"><p><strong>'.__('WooCommerce customer fields have been added successfully.','users-ultra').'</strong></p></div>';
